...
(function($) {
	...
	if (!$.multiUpload.initialized && getFlashVersion() >= 10) {
		...
		$.extend($.multiUpload.prototype, {
			...
			init : function() {
				var up = this, s = up.settings, so, fb, filter;
				...
				$(document.body).append(
					...
					'<object id="flashuploader" width="100%" height="100%" type="application/x-shockwave-flash" data="js/jquery/jquery.multiupload.flash.swf">' +
					'<embed src="js/jquery/jquery.multiupload.flash.swf" type="application/x-shockwave-flash" allowscriptaccess="always" width="100%" height="100%"></embed>' + 
					'<param name="movie" value="js/jquery/jquery.multiupload.flash.swf" />' +
