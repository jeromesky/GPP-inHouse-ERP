...
<cfoutput>
<div id="header">#get.eventName# <cfif parameterexists(form.count)>Count: #get.recordcount#</cfif></div>
...
<!---      <tr>
    ...
    <cfloop query="get"><tr>
        ...
        <cfif form.paid EQ "yes" OR form.paid EQ "no" OR form.paid EQ "all">
          <td valign="bottom"><cfif get.paid EQ "yes">Yes<cfelse>No</cfif></td>
        </cfif>
        <!--- <cfif form.ChkCancelled EQ "= 1" OR form.ChkCancelled EQ "*">
      <td><cfif get.ChkCancelled EQ 1>Cancel<cfelse>Attended</cfif></td></cfif> --->
        <cfif parameterexists(form.chkCompGuest)><td><cfif get.chkCompGuest EQ 1 >Yes</cfif></td>
        </cfif>
        <cfif parameterexists(form.attend_type)>
        ...
        <cfif parameterexists(form.discount)><td><cfif get.discount EQ 1 >Yes</cfif></td></cfif>
        <td>#get.invoiceID#</td>
      </tr></cfloop>
