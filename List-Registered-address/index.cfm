...
FROM tblAnnual_Conf
WHERE (event_type = 'event' OR event_type =  'din' OR event_type = 'eb' OR event_type = 'Bu_pr' OR AnType = 'exh' OR AnType = 'spon' OR AnType = 'list' OR AnType = 'Book' ) 
AND ( event_expire > #DateAdd("yyyy", -14, now())#  OR dtmConference_Date > #DateAdd("yyyy", -14, now())#)
